...
@DATASETS.register_module()
class RepeatPartDataset(object):
    """A wrapper of dataset that requires partial repetition.

    Args:
        dataset (:obj:`CustomDataset`): The dataset to be class sampled.
        times (int): Repeat times.
        partial_classes (tuple): Classes requiring repetition.
        class_sample_idxs_path (str): The path of class_sample_idxs
    """
    ...
    def __init__(self,
                 ...
                 times,
                 partial_classes,
                 class_sample_idxs_path=None):
        self.dataset = dataset
        self.times = times
        self.CLASSES = dataset.CLASSES
        self.PALETTE = dataset.PALETTE
        self.partial_classes = partial_classes
        self.class_sample_idxs_path = class_sample_idxs_path
    ...
    def _get_sample_indices(self):
        ...
        """
        if self.class_sample_idxs_path is None:
            class_sample_idxs = {
                self.CLASSES.index(class_name): []
                for class_name in self.partial_classes
            }
            for idx in range(len(self.dataset)):
                sample_cat_ids = self.dataset.get_cat_ids(
                    idx, self.partial_classes)
                for cat_id in sample_cat_ids:
                    class_sample_idxs[cat_id].append(idx)
        else:
            raw_class_sample_idxs = mmcv.load(self.class_sample_idxs_path)
            class_sample_idxs = {}
            for class_name in self.partial_classes:
                class_idx = self.CLASSES.index(class_name)
                class_sample_idxs[class_idx] = raw_class_sample_idxs[class_idx]

        sample_indices = list(range(len(self.dataset)))

        for cls_inds in list(class_sample_idxs.values()):
            sample_indices += cls_inds * (self.times - 1)
