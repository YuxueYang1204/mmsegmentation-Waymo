    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        with torch.no_grad():
            ...
            for single_data in data:
                result.extend(model(return_loss=False, **single_data))
        ...
        if format_only:
            dataset.format_results(
                result, img_metas=data[0]['img_metas'], **format_args)
        # if pre_eval:
...
def multi_gpu_test(model,
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        with torch.no_grad():
            result = []
            for single_data in data:
                result.extend(model(return_loss=False,
                                    rescale=True,
                                    **single_data))
...
def main():
    ...
    if args.aug_test:
        # hard code index
        cfg.data.infer.pipeline[1].img_ratios = [
            0.5, 0.75, 1.0, 1.25, 1.5, 1.75
        ]
        cfg.data.infer.pipeline[1].flip = True
